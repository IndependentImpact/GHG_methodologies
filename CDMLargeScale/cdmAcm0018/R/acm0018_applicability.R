...
#' Check ACM0018 technology type applicability (semantic)
#'
...
check_applicability_technology_type <- function(data,
  ...
  shapes          <- if (is.null(shapes)) read_acm0018_technology_shapes() else shapes
  ...
  cdmSemantic::cdm_make_applicability_result(result, data, "ACM0018", "RenewableBiomassSystem")
}

#' Check ACM0018 grid connection applicability (semantic)
#'
...
check_applicability_grid_connection <- function(data,
                                                ...
                                                shapes = NULL) {
  triples         <- cdmSemantic::cdm_resolve_triples(data, fluree_conn)
  shapes          <- if (is.null(shapes)) read_acm0018_grid_shapes() else shapes
  concept_triples <- if (is.null(concept_triples)) cdmSemantic::read_cdm_concept_triples() else concept_triples
  augmented <- shaclR::materialise_skos_hierarchy(triples, concept_triples)
  result    <- shaclR::validate_shacl(augmented, shapes)
  cdmSemantic::cdm_make_applicability_result(result, data, "ACM0018", "GridConnectedSystem")
}
