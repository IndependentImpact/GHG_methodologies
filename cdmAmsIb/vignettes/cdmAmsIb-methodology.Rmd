...
function_reference <- tibble::tibble(
  ...
  `Equation (LaTeX)` = c(
    "\\(Q_{BL,mp} = \\sum_{i} FC_{i,mp} \\times NCV_i\\)",
    "\\(E_{BL,mp}^{CO2} = Q_{BL,mp} \\times EF_{BL}\\)",
    "\\(E_{PR,mp}^{CO2} = Q_{PR,mp} \\times EF_{PR}\\)",
    "\\(ER_{mp} = E_{BL,mp}^{CO2} - E_{PR,mp}^{CO2}\\)",
    "\\(ER_{mp}^{agg} = \\sum_{i} ER_{i,mp}\\)",
    "\\(ER_{total} = \\sum_{mp} ER_{mp}\\)",
    "\\(FC_{i,mp} = Q_{BL,mp} / NCV_i\\)"
  ),
  Purpose = c(
    "Convert baseline fuel consumption into mechanical energy.",
    "Calculate baseline emissions using fossil fuel factors.",
    "Estimate project emissions from any residual fossil use.",
    "Compute emission reductions by differencing baselines and project values.",
    "Aggregate monitoring data to reporting periods.",
    "Provide a wrapper returning emission reductions and supporting columns.",
