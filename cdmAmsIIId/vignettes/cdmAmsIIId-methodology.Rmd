...
tibble::tibble(
  ...
  knitr::kable(caption = "Applicability outcomes for each AMS-III.D criterion")
```
...
```{r simulated-data}
head(monitoring) |>
  knitr::kable(caption = "Excerpt of simulated AMS-III.D monitoring data")
```
...
list(
  ...
  knitr::kable(caption = "Equation-level outputs across components")
```
...
aggregate_monitoring_periods_iiid(monitoring, group_cols = "farm_id") |>
  knitr::kable(caption = "Aggregated monitoring data by farm")
```
...
reductions |>
  knitr::kable(caption = "Net emission reductions from AMS-III.D workflow")
```
...
knitr::kable(overview, caption = "High-level AMS-III.D workflow")
```
