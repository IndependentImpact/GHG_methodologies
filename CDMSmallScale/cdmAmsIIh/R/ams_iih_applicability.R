...
#' Check AMS-II.H energy efficiency technology applicability (semantic)
#'
#' Validates that the project activity implements an energy efficiency technology,
#' as required by AMS-II.H condition (a). Uses SHACL validation against the
...
#'   [read_ams_iih_technology_shapes()].
#'
#' @return A list with:
#' \describe{
...
check_applicability_technology_type <- function(data,
  ...
  shapes          <- if (is.null(shapes)) read_ams_iih_technology_shapes() else shapes
  ...
  cdmSemantic::cdm_make_applicability_result(result, data, "AMS-II.H", "EnergyEfficiencyTechnology")
}

#' Check AMS-II.H facility type applicability (semantic)
#'
#' Validates that the project activity is implemented at a centralized utility facility,
#' as required by AMS-II.H condition (b).
#'
#' @param data Either a single character project IRI or a data frame with
...
check_applicability_facility_type <- function(data,
                                              ...
                                              shapes = NULL) {
  triples         <- cdmSemantic::cdm_resolve_triples(data, fluree_conn)
  shapes          <- if (is.null(shapes)) read_ams_iih_facility_shapes() else shapes
  concept_triples <- if (is.null(concept_triples)) cdmSemantic::read_cdm_concept_triples() else concept_triples
  augmented <- shaclR::materialise_skos_hierarchy(triples, concept_triples)
  result    <- shaclR::validate_shacl(augmented, shapes)
  cdmSemantic::cdm_make_applicability_result(result, data, "AMS-II.H", "CentralizedUtilityFacility")
}
