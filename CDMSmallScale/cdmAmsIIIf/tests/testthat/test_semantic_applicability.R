...
test_that("check_applicability_technology_type returns required list names", {
  skip_if_no_shapeR()
  skip_if_no_rdflib()
  triples <- .make_triples(paste0(CDM, "AerobicComposting"))
...
test_that("attestation has correct methodology and condition", {
  ...
  triples <- .make_triples(paste0(CDM, "AerobicComposting"))
  result  <- check_applicability_technology_type(triples)
  expect_equal(result$attestation$methodology, "AMS-III.F")
  expect_equal(result$attestation$condition, "AerobicComposting")
})

test_that("AerobicComposting typed as intermediate node passes", {
  skip_if_no_shapeR()
  skip_if_no_rdflib()
  triples <- .make_triples(paste0(CDM, "AerobicComposting"))
  result  <- check_applicability_technology_type(triples)
...
test_that("WasteTreatmentTechnology (top concept) typed as intermediate node passes after materialisation", {
  skip_if_no_shapeR()
  skip_if_no_rdflib()
  triples <- .make_triples(paste0(CDM, "AerobicComposting"))
  result  <- check_applicability_technology_type(triples)
...
test_that("MunicipalOrganicWaste typed as intermediate waste node passes", {
  skip_if_no_shapeR()
  skip_if_no_rdflib()
  triples <- .make_triples(
    paste0(CDM, "AerobicComposting"),
    waste_type = paste0(CDM, "MunicipalOrganicWaste")
  )
  result <- check_applicability_waste_type(triples)
  expect_true(result$conforms)
})

test_that("waste_type attestation condition is OrganicWaste", {
  skip_if_no_shapeR()
  skip_if_no_rdflib()
  triples <- .make_triples(
    paste0(CDM, "AerobicComposting"),
...
test_that("RecyclableMaterial typed as intermediate waste node fails", {
  ...
  triples <- .make_triples(
    paste0(CDM, "AerobicComposting"),
    waste_type = paste0(CDM, "RecyclableMaterial")
  )
  result <- check_applicability_waste_type(triples)
  expect_false(result$conforms)
})
