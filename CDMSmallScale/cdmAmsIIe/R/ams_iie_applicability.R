...
#' Check AMS-II.E energy efficiency technology applicability (semantic)
#'
#' Validates that the project activity implements an energy efficiency technology,
#' as required by AMS-II.E condition (a). Uses SHACL validation against the
...
#'   [read_ams_iie_technology_shapes()].
#'
#' @return A list with:
#' \describe{
...
check_applicability_technology_type <- function(data,
  ...
  shapes          <- if (is.null(shapes)) read_ams_iie_technology_shapes() else shapes
  ...
  cdmSemantic::cdm_make_applicability_result(result, data, "AMS-II.E", "EnergyEfficiencyTechnology")
}

#' Check AMS-II.E facility type applicability (semantic)
#'
#' Validates that the project activity is implemented at a building,
#' as required by AMS-II.E condition (b).
#'
#' @param data Either a single character project IRI or a data frame with
...
check_applicability_facility_type <- function(data,
                                              ...
                                              shapes = NULL) {
  triples         <- cdmSemantic::cdm_resolve_triples(data, fluree_conn)
  shapes          <- if (is.null(shapes)) read_ams_iie_facility_shapes() else shapes
  concept_triples <- if (is.null(concept_triples)) cdmSemantic::read_cdm_concept_triples() else concept_triples
  augmented <- shapeR::materialise_skos_hierarchy(triples, concept_triples)
  result    <- shapeR::validate_shacl(augmented, shapes)
  cdmSemantic::cdm_make_applicability_result(result, data, "AMS-II.E", "Building")
}
