...
function_reference <- tibble::tibble(
  ...
  `Equation (LaTeX)` = c(
    "\\(Q_{BL,mp} = \\sum_{i} E_{i,mp}\\)",
    "\\(E_{BL,mp}^{CO2} = Q_{BL,mp} \\times EF_{BL}\\)",
    "\\(E_{PR,mp}^{CO2} = Q_{BL,mp} \\times EF_{PR}\\)",
    "\\(ER_{mp} = E_{BL,mp}^{CO2} - E_{PR,mp}^{CO2}\\)",
    "\\(ER_{mp}^{agg} = \\sum_{i} ER_{i,mp}\\)",
    "\\(ER_{total} = \\sum_{mp} ER_{mp}\\)",
    "\\(E_{i,mp} \\sim \\mathcal{N}(\\mu_{E}/P, \\sigma_{E}/\\sqrt{P})\\)"
  ),
  Purpose = c(
    "Summed baseline electricity supply for each group or monitoring period.",
    "Baseline emissions derived from fossil electricity emission factors.",
    "Project emissions reflecting auxiliary fossil generation.",
